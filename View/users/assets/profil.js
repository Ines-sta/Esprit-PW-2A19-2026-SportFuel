window.addEventListener('DOMContentLoaded', () => {
  ...
  if (profilePhotoInput && profileAvatar) {
    profilePhotoInput.addEventListener('change', function () {
      ...
      fetch('/Esprit-PW-2A19-2026-SportFuel-main/Controller/api/api.php?action=upload_profile_photo', {
        method: 'POST',
        body: payload
      })
  ...
  window.saveProfile = function() {
    ...
    fetch('/Esprit-PW-2A19-2026-SportFuel-main/Controller/api/api.php?action=save_profil', {
      method: 'POST',
      headers: {
  ...
  window.deleteAccount = function() {
    if (confirm("Voulez-vous VRAIMENT supprimer votre compte ? Cette action est totalement irréversible.")) {
      fetch('/Esprit-PW-2A19-2026-SportFuel-main/Controller/api/api.php?action=delete_account', { method: 'POST' })
      .then(r => r.json())
      .then(res => {
         if (res.success) {
            alert('Compte supprimé définitivement.');
          window.location.href = '/Esprit-PW-2A19-2026-SportFuel-main/index.php?page=auth&action=logout';
         } else {
