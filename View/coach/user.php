...
if ($usingCanonicalCoachRoute) {
    $current_page = '/Esprit-PW-2A19-2026-SportFuel-main/index.php?page=coach';
...
$answeredPublications = 0;
if (is_array($publications)) {
    foreach ($publications as $publicationItem) {
        $comments = isset($publicationItem['commentaires']) && is_array($publicationItem['commentaires'])
            ? $publicationItem['commentaires']
            : [];
        foreach ($comments as $commentItem) {
            if ((int)($commentItem['id_utilisateur'] ?? 0) !== (int)($current_user['id_utilisateur'] ?? 0)) {
                $answeredPublications++;
                break;
            }
        }
    }
}
$pendingPublications = max(0, $totalPublications - $answeredPublications);
...
<head>
    ...
    <link rel="stylesheet" href="/Esprit-PW-2A19-2026-SportFuel-main/public/css/coach.css">
    <style>
        ...
        .request-main-card { max-width: none; margin: 0 0 20px; background: #fff; border: 1px solid #d9eee2; border-radius: var(--radius); box-shadow: var(--shadow); padding: 22px; }
        ...
        .selected-files-list { margin: 0; padding-left: 18px; color: #2f4a3f; font-size: 0.9em; }
        ...
        .suivi-history { background: #fff; border: 1px solid #d9eee2; border-radius: var(--radius); box-shadow: var(--shadow); padding: 20px; margin-bottom: 20px; }
        .suivi-history h2 { font-weight: 800; letter-spacing: 0.35px; margin-bottom: 14px; color: var(--vert-foret); }
        .coach-user-page .page-header { margin-top: 0; margin-bottom: 16px; }
        .coach-user-page .welcome-banner { margin-top: 0; }
        .status-stats .stat-card .value { color: var(--vert-vif); }
        .status-stats .stat-card .value.orange { color: var(--orange-energie); }
        .status-stats { margin-bottom: 20px; }
...
<div class="main-content coach-user-page">
    <div class="page-header">
        <div>
            <h1><?php echo htmlspecialchars($page_title); ?></h1>
            <div class="page-date">Espace de suivi avec coach</div>
        </div>
    </div>

    <div class="welcome-banner">
        <div>
            <p class="greeting">Publication & Suivi</p>
            <h2>Centralisez vos demandes</h2>
            <p class="sub">Déposez vos questions entraînement/nutrition et suivez les réponses de votre coach dans un seul espace.</p>
        </div>
    </div>

    <div class="stat-cards status-stats">
        <div class="stat-card">
            <div class="value"><?php echo (int)$totalPublications; ?></div>
            <div class="label">Demandes publiées</div>
        </div>
        <div class="stat-card">
            <div class="value"><?php echo (int)$answeredPublications; ?></div>
            <div class="label">Demandes répondues</div>
        </div>
        <div class="stat-card">
            <div class="value orange"><?php echo (int)$pendingPublications; ?></div>
        ...
        <form id="add-pub-form" action="<?php echo htmlspecialchars($current_page); ?>" method="POST" enctype="multipart/form-data" novalidate>
            ...
            <div class="form-group">
                ...
                <div class="upload-area">
                    <div id="upload-dropzone" class="dropzone" tabindex="0" role="button" aria-label="Déposer des fichiers ou cliquer pour parcourir">
                        <div class="dropzone-title">Glissez-déposez vos fichiers ici</div>
                        <div class="dropzone-subtitle">ou cliquez pour parcourir (images, vidéos, PDF)</div>
                    </div>
                    <input id="attachments" name="attachments[]" type="file" class="file-input file-input-hidden" accept="image/*,video/*,application/pdf" multiple>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (addForm) {
            ...
            if (attachmentsInput && filesList) {
                attachmentsInput.addEventListener('change', refreshSelectedFiles);
            }

            if (attachmentsInput && uploadDropzone) {
