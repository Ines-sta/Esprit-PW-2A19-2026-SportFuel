document.addEventListener('DOMContentLoaded', function () {
    const API = '../../Controller/api/api.php';
    ...
    async function startFaceIDLogin() {
        ...
        let dbDescriptors = [];
        try {
            const response = await fetch('../../Controller/api/api.php?action=get_face_descriptors', {
                credentials: 'include'
            });
        ...
        const interval = setInterval(async () => {
            ...
            if (detection) {
                ...
                if (bestMatch && minDistance < 0.45) { // Seuil strict pour la DB
                    ...
                    try {
                        const loginRes = await fetch('../../Controller/api/api.php?action=login_by_id', {
                            method: 'POST',
                            credentials: 'include',
                            headers: { 'Content-Type': 'application/json' },
