...
if ($action === 'send_reset_code' && $method === 'POST') {
    ...
    require __DIR__ . '/../shared/smtp_settings.php';
...
if ($action === 'login_by_id' && $method === 'POST') {
    ...
    if ($user) {
        ...
        echo json_encode([
            ...
            'redirect' => sportfuel_canonical_redirect_path($user['role'] ?? 'Sportif'),
            'legacy_redirect' => sportfuel_legacy_redirect_path($user['role'] ?? 'Sportif')
        ]);
    } else {
