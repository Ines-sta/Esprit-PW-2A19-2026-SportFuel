...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action'])) {
    ...
    $basePath = '/Esprit-PW-2A19-2026-SportFuel-main';
    
    if ($_GET['action'] === 'inscription') {
        $result = $auth->inscription($_POST);
        if ($result['success']) {
            $redirect = sportfuel_canonical_redirect_path($result['role'] ?? 'Sportif');
            header('Location: ' . $redirect);
            exit();
        } else {
            echo "<script>alert('".$result['message']."'); window.location.href='" . $basePath . "/View/auth/inscription.html';</script>";
        }
    }
    ...
    if ($_GET['action'] === 'connexion') {
        ...
        if ($result['success']) {
            $redirect = sportfuel_canonical_redirect_path($result['role'] ?? 'Sportif');
            $legacyRedirect = sportfuel_legacy_redirect_path($result['role'] ?? 'Sportif');
            if ($isAjax) {
                echo json_encode([
                    'success' => true,
                    'redirect' => $redirect,
            ...
            if ($isAjax) {
                ...
                echo "<script>alert('".$result['message']."'); window.location.href='" . $basePath . "/View/auth/connexion.html';</script>";
            }
        }
    }
